...
def add_new_supplement():
    ...
    bdf = pd.read_csv(r"file:///D:/IP/Supplements.csv")
    n = bdf["supplement_id"].count()
    bdf.at[n] = [supplement_id, supplement_name, cost, category]
    bdf.to_csv(r"file:///D:/IP/Supplements.csv", index=False)
...
def add_new_player():
    ...
    bdf = pd.read_csv(r"file:///D:/IP/player.csv")
    n = bdf["player_id"].count()
    bdf.at[n] = [player_id, name, stage, year_of_joining]
    bdf.to_csv(r"file:///D:/IP/player.csv", index=False)
...
def issue_supplements():
    ...
    mdf = pd.read_csv(r"file:///D:/IP/player.csv")
    mdf = mdf.loc[mdf["player_name "] == p_name]
    if mdf.empty:
        print("No Such Member found:(")
        return

    delete_of_issue = int(input("Enter Date of issue:"))
    number_of_supplements_issued = int(input("enter  number of book issued:"))
    bdf = pd.read_csv(r"file:///D:/IP/issue.csv")
    n = bdf["supplement_name"].count()
    bdf.at[n] = [supplement_name, p_name, date.today(), number_of_supplements_issued, ""]
    bdf.to_csv(r"file:///D:/IP/issue.csv", index=False)
...
def return_supplement():
    ...
    supplement_name = input("Enter supplement Name :")
    idf = pd.read_csv(r"file:///D:/IP/issue.csv")
    ...
    if idf.empty:
        ...
        if idf.empty:
            ...
            if ans.lower() == "yes":
                idf = pd.read_csv(r"file:///D:/IP/issue.csv")
                idf = idf.drop(idf[idf["supplement_name"] == supplement_name].index)
                idf.to_csv(r"file:///D:/IP/issue.csv", index=False)
...
def show_charts():
    ...
    if char == 1:
        df = pd.read_csv(r"file:///D:/IP/Supplements.csv")
    ...
    if ch == 2:
        df = pd.read_csv(r"file:///D:/IP/issue.csv")
        df = df[["number of supplements issued"]]
...
def login():
    ...
    df = pd.read_csv(r"file:///D:/IP/user.csv")
